...
{
	/// <summary>This class adds context menu options to the SpriteShapeRenderer component for integration with Destructible2D.</summary>
	public static class D2dDestructibleSpriteShape
	{
		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible", true)]
		private static bool MakeDestructibleValidate(MenuCommand menuCommand)
		{
			var gameObject = GetGameObject(menuCommand);

			return gameObject.GetComponent<SpriteShapeRenderer>() != null && gameObject.GetComponent<D2dDestructibleRenderer>() == null;
		}

		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible", false)]
		private static void MakeDestructible(MenuCommand menuCommand)
		{
			var gameObject = GetGameObject(menuCommand);

			AddSingleComponent<D2dDestructibleRenderer>(gameObject, d => { d.ChangeMaterial(); d.Rebuild(); });
		}

		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible (Preset: Dynamic Splittable)", true)]
		private static bool MakeDestructibleDynSplValidate(MenuCommand menuCommand)
		{
			return AddSingleComponentValidate<D2dDestructibleRenderer>(GetGameObject(menuCommand));
		}

		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible (Preset: Dynamic Splittable)", false)]
		...
		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible (Preset: Dynamic Detachable)", true)]
		private static bool MakeDestructibleDynDetValidate(MenuCommand menuCommand)
		{
			return AddSingleComponentValidate<D2dDestructibleRenderer>(GetGameObject(menuCommand));
		}

		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible (Preset: Dynamic Detachable)", false)]
		...
		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible (Preset: Dynamic Solid)", true)]
		private static bool MakeDestructibleDynSolValidate(MenuCommand menuCommand)
		{
			return AddSingleComponentValidate<D2dDestructibleRenderer>(GetGameObject(menuCommand));
		}

		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible (Preset: Dynamic Solid)", false)]
		private static void MakeDestructibleDynSol(MenuCommand menuCommand)
		{
			var gameObject = GetGameObject(menuCommand);

			AddSingleComponent<D2dDestructibleRenderer>(gameObject, c => { c.ChangeMaterial(); c.Rebuild(); });
			AddSingleComponent<D2dEdgeCollider>(gameObject);
		}

		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible (Preset: Static)", true)]
		private static bool MakeDestructibleStaValidate(MenuCommand menuCommand)
		{
			return AddSingleComponentValidate<D2dDestructibleRenderer>(GetGameObject(menuCommand));
		}

		[MenuItem("CONTEXT/SpriteShapeRenderer/Make Destructible (Preset: Static)", false)]
